...
def main(argv):
    ...
    ### Argument parsing ###
    ########################
    USAGE = '%s <subj> <smoothness regularization> <field regularization> <out folder> [pca]' % argv[0]
    ...
    for sigma in [8]:
        ...
        basic_threshold_segmentation_stroke = dataset.get(subj=subj, modality='dwi', feature='stroke_raw_threshold_seg', modifiers='')
        ...
        if warp_pca:
            for pc in xrange(1,44):
                label_warp = pb.ANTSWarpCommand.make_from_registration(
                        "Warp atlas labels to subject space",
                        moving=buckner.get_original(feature='_pcav6_%d'%pc),
                        reference=subj_final_img,
                        registration=forward_reg,
                        useNN=False,
                        )
        ...
        #         "Threshold segmentation for stroke (volume)",
        #         infile=dwi_matchwm,
        ...
        # stroke_in_flair = dataset.get(subj=subj, modality='flair', feature='stroke', modifiers='')
        # stroke_to_flair = pb.ANTSWarpCommand.make_from_registration(
        #         "Warp stroke segmentation to FLAIR",
        #         moving=basic_threshold_segmentation_stroke,
        #         reference=subj_final_img,
        #         output_filename=stroke_in_flair,
        #         registration=multimodal_registration,
        #         inversion='forward',
        #         )
        ...
        #         "Threshold segmentation for stroke (scalar)",
        #         infile=dwi_matchwm,
        #         threshold=STROKE_THRESHOLD,
        ...
        threshold_segmentation_count = pb.NiiToolsMaskedThresholdCountCommand(
                "Threshold segmentation for WMH (scalar)",
                infile=intensity_corr.outfiles[0],
                threshold=WMH_THRESHOLD,
